...
for index, data_file in enumerate(data):
    ...
    for nr in range(Nramek):
        ...
        frame_energy = np.sum(bx ** 2) / Mlen

        # Voiced/unvoiced decision based on energy
        energy_threshold = 5500e3  # This threshold might need tuning
        T = 0 if frame_energy < energy_threshold else imax  # Voiced/unvoiced decision
        print(f"Frame energy: {frame_energy}, T: {T}")
        ...
        for n in range(Mstep):
            if T == 0:      # sprawdzenie czy mowa jest dźwięczna
    ...
    wavfile.write(f'synthesized_{data_file}_better.wav', fs, s)
