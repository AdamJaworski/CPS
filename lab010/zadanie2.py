...
for index, data_file in enumerate(data):
    ...
    for nr in range(Nramek):
        ...
        # T = imax if rmax > 0.35 * r[0] else 0
        # A #T = 0 if rmax > 0.35 * r[0] else 0      # zawsze szeptanie
        # B #T = imax/x if rmax > 0.35 * r[0] else 0 # tłumienie
        # C #T = 80 if rmax > 0.35 * r[0] else 0     # robotlike
        # D #T = 0 if rmax > 0.35 * r[0] else 0
        rr = r[1:Np + 1]
    ...
    wavfile.write(f'synthesized_{data_file}_zad2a.wav', fs, s)
